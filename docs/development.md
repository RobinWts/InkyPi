...
1. **Install InkyPi on the Pi with mock display** (recommended): clone the repo, then run:
   ```bash
   sudo bash install/install.sh -M
   ```
   The `-M` option installs for **mock display**: it uses a config with `display_type: mock`, resolution 800×480, and output in `/usr/local/inkypi/mock_display_output`, and skips SPI/I2C setup. The app does not load Inky or Waveshare drivers, so it won’t crash when no display is connected.

   **Alternatively**, install as usual (`sudo bash install/install.sh` or with `-W <waveshare_model>`), then edit `/usr/local/inkypi/src/config/device.json` and add `"display_type": "mock"` (and optionally `"resolution": [800, 480]`, `"output_dir": "/usr/local/inkypi/mock_display_output"`). If `resolution` is missing, 800×480 is used.

2. **Restart the service** (if you already had InkyPi installed and only changed config):
   ```bash
   sudo systemctl restart inkypi.service
   ```

3. **Use the web UI** from another device at `http://<pi-ip-address>` (port 80). Open **Manage plugins** (puzzle icon), install or uninstall a third-party plugin; the service will restart after each action. Use **Reload page** once the service is back.

Rendered images are written to the device (e.g. `src/static/images/current_image.png`); with mock display they are also written to the mock output path (by default `/usr/local/inkypi/mock_display_output`). To switch back to a real display later, set `display_type` to `"inky"` or the Waveshare model (e.g. `"epd7in3f"`) in `device.json` and restart the service.
